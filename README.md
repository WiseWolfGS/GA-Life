...
- 유전자와 영향력
  ...
  [4, 4, 5]]
   ...
   이런 규칙을 따르는 만큼 주변 살아있는 세포가 주는 영향력의 총합인 S\_total은 지형 등 다른 보정 없이는 최대 76이다.
...
- 연속 확률 모델
  ...
  생존 확률: p_live(S_total) = clip(α * {σ(β * (S_total - μ))} * {1 - σ(β * (S_total - μ))}^γ + δ, 0, 1)

  사망 확률: p_dead(S_total) \= 1 - p_live(S_total)

  탄생 확률: p_born(S_total) = clip(α * {σ(β * (S_total - ν))} * {1 - σ(β * (S_total - ν))}^γ + δ, 0, 1)

  미탄생 확률: p_unborn(S_total) \= 1 - p_born(S_total)
...
- 연속 확률 모델의 예외 사항
   ...
   만약 p_live(S_total)과 p_born(S_total)이 ε(기본 값은 0.00072)보다 작을 경우 무조건 0으로 계산한다.
